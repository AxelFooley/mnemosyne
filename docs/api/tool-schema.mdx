...
# MCP Tool Schema (v4.0.0b3)

> **Generated file. Do not edit by hand.**
> Source: `mnemosyne/tool_schemas.py` and `mnemosyne/mcp_tools.py`.
> Regenerate with `python3 scripts/generate-docs.py`.

Mnemosyne declares **37 tools**. Of those, **29 are callable over MCP** (stdio, SSE and Streamable HTTP), and **8 are implemented only in the Hermes provider** and are not reachable through the MCP server.
...
| `store` | `enum[private, surface]` | no | `private` |
| `bank` | `string` | no | *(none)* |
